...
gg1 <- ggplot(mtcars, aes(x=log.weight, y=mpg, col=transmission)) +
    ...
    labs(title="MPG vs. Weight and Transmission Type") + 
...
bestmodel <- fit[[tail(which(fit.anova$"Pr(>F)" <= .05),n=1)]]
bestmodel.str <- deparse(bestmodel$call)
bestmodel.str <- paste0(bestmodel.str[1],bestmodel.str[2])
```

The ANOVA results show that the model `r bestmodel.str` will provide an optimal level of modeling without excess variance inflation.
...
```{r results.residuals, echo=FALSE, fig.width=6.5}
...
```{r results.confidence, results='asis'}
